...
def wrap_copilot_prompt(message: str, req_id: str) -> str:
    """Wrap a prompt with CCB protocol markers for Copilot."""
...
def extract_reply_for_req(text: str, req_id: str) -> str:
    """
    Extract the reply segment for req_id from a Copilot message.

    Copilot may emit multiple replies in a single assistant message, each ending with its own
