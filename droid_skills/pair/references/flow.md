...
Recommended: set `CCB_CALLER=droid` when invoking `ask` so completion hooks attribute correctly.

If you need a raw example for Linux/macOS/WSL:
```bash
CCB_CALLER=droid ask <provider> --background <<'EOF'
<message>
EOF
```
